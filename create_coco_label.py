...
# 经过上面步骤，已经安装好了pycocotools到python库了，进入python3命令行就可以直接导入了，进入python环境执行下面命令 import pycocotools不报错说明已经安装成功了


# 第二步:
# 将annotations文件夹拷贝到train2014同级目录下
# 在train2014同级目录下新建一个类别文件coco_list.txt（需要的类别，格式如下）
'''
0,person
1,bicycle
2,car
...
dataDir = '/home/loulinfeng/llf/darknet/data/coco/images/train2014'  # COCO数据集所在的路径
#dataDir = '/home/loulinfeng/llf/darknet/data/coco/images/val2014'  

dataType = 'train2014'  # 要转换的COCO数据集的子集名
#dataType = 'val2014' 

annFile = '/home/loulinfeng/llf/darknet/data/coco/images/annotations/instances_%s.json' % (dataType)  # COCO数据集的标注文件路径
classes = get_classes_and_index('/home/loulinfeng/llf/darknet/data/coco/coco_list.txt')
...
if not os.path.exists('%s/filelist/' % (dataDir)):
    ...
    shutil.rmtree('%s/filelist/' % (dataDir))
    os.makedirs('%s/filelist/' % (dataDir))
...
for imgId in imgIds:
    ...
    for annId in annIds:
        ...
        if cat in classes:
            objCount = objCount + 1
            out_file = open('%s/labels/%s.txt' % (dataDir, filename[:-4]), 'a')
